...
const validateServiceId = (req, res, next) => {
  try {
    ...
    if (!Number.isInteger(id) || id <= 0) {
      throw new AppError("Service id must be a positive integer", 400);
    }
    next();
  } catch (error) {
    next(error);
...
const validateCreateService = (req, res, next) => {
  try {
    ...
    validateText(req.body.description, "Description", { required: true, min: 2, max: 500 });
    next();
  } catch (error) {
    next(error);
  }
...
const validateUpdateService = (req, res, next) => {
  try {
    ...
    validateText(req.body.description, "Description", { required: false, min: 2, max: 500 });
    next();
  } catch (error) {
    next(error);
  }
