// src/utils/gst.js
// ─────────────────────────────────────────────────────────────────────────
// CLIENT MIRROR of backend src/utils/gst.engine.js computeDocument().
//
// This is a VERBATIM port of the server-authoritative GST engine, kept here
// ONLY for live UX (instant totals as the user types). The server recomputes
// the exact same math at save/finalize and ALWAYS wins — if this ever drifts
// from the backend engine, the backend is the source of truth. Keep the math
// (round2, supply type, per-line breakup, totals, rounding modes, RCM excludes
// tax, no_tax, inclusive/exclusive) byte-for-byte aligned with the backend.
//
// Money model: every numeric input is coerced with Number() and every monetary
// output is guarded with round2() to avoid binary-float drift, matching the
// server (which also has to coerce mysql2 DECIMAL-as-string values).
//
 ...
 * fractional part is rounded to 2 decimals (paise) before conversion.
 *
 * NOTE: amount-in-words is advisory client-side only; the backend value (also
 * produced by this same algorithm) is authoritative on the stored/printed doc.
 * @param {number} amount
 * @returns {string}
 */
export const numToIndianWords = (amount) => {
  ...
  const rupeeWords = rupees > 0 ? `${integerToIndianWords(rupees)} ` : "";
...
const lineDiscount = (gross, discountType, discountValue) => {
  ...
  const discount = discountType === "percent" ? (gross * value) / 100 : value;
 ...
 * in words. Pure.
 *
 * DIFFERENCE FROM BACKEND: where the server throws AppError(422) when the
 * state codes are missing, this client mirror returns null gracefully (the
 * builder is mid-edit before a party is picked) — callers should treat a null
 * result as "not yet computable". The math is otherwise identical.
 *
 * @param {Object} input
 ...
 * @returns {Object|null} { supplyType, items, totals, amountInWords } | null
 */
export const computeDocument = (input = {}) => {
  const {
    sellerStateCode,
  ...
  // Backend throws here; client returns null so the UI can stay neutral until
  // the seller state + place of supply are both known.
  if (!seller || !pos) {
    return null;
  }
